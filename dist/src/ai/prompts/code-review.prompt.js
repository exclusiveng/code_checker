...
exports.generateSubmissionSummaryPrompt = generateSubmissionSummaryPrompt;
exports.generateFixSuggestionPrompt = generateFixSuggestionPrompt;
/**
 * Generate a code review prompt
 */
function generateCodeReviewPrompt(params) {
    const { files, projectContext, existingRules, focusAreas } = params;
    const filesList = files.map((f, idx) => `### File ${idx + 1}: ${f.path}\n\`\`\`\n${f.content.substring(0, 5000)}\n\`\`\`\n`).join('\n\n');
 ...
 */
function generateSubmissionSummaryPrompt(params) {
 ...
 */
function generateFixSuggestionPrompt(params) {
