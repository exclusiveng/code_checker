...
exports.generateRulesetPrompt = generateRulesetPrompt;
exports.generateRuleSuggestionsPrompt = generateRuleSuggestionsPrompt;
exports.generateRulesetImprovementPrompt = generateRulesetImprovementPrompt;
/**
 * Generate ruleset from natural language prompt
 */
function generateRulesetPrompt(input) {
 ...
 */
function generateRuleSuggestionsPrompt(input) {
    ...
    const issuesList = topIssues.map((issue, idx) => `${idx + 1}. [${issue.category}] ${issue.issue} (occurred ${issue.frequency} times)`).join('\n');
 ...
 */
function generateRulesetImprovementPrompt(params) {
