...
exports.generateSubmissionReportPrompt = generateSubmissionReportPrompt;
exports.generateProjectInsightsPrompt = generateProjectInsightsPrompt;
function generateSubmissionReportPrompt(input) {
    const { submission, projectContext } = input;
    const findings = submission.results?.findings || [];
    const findingsSummary = findings.map((f) => `- [${f.severity}] ${f.message} (File: ${f.locations?.[0]?.file || 'unknown'})`).join('\n');
...
function generateProjectInsightsPrompt(input) {
