...
exports.BackfillProjectSlugs1680000000000 = void 0;
...
class BackfillProjectSlugs1680000000000 {
    async up(queryRunner) {
        // Backfill slugs for rows that have null or empty slug
        const projects = await queryRunner.query(`SELECT id, name, company_id FROM projects WHERE slug IS NULL OR slug = ''`);
        for (const p of projects) {
            let base = generateSlug(p.name || `project-${p.id.slice(0, 6)}`);
            let slug = base;
            let attempt = 0;
            // check uniqueness within company
            while ((await queryRunner.query(`SELECT 1 FROM projects WHERE slug = $1 AND company_id = $2`, [slug, p.company_id])).length > 0) {
                attempt++;
...
exports.BackfillProjectSlugs1680000000000 = BackfillProjectSlugs1680000000000;
