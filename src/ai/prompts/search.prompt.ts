...
export interface SubmissionComparisonInput {
  submission1: {
    id: string;
    code: string;
    results: any;
  ...
  comparisonFocus?: 'quality' | 'performance' | 'security' | 'all';
}
...
export function generateCodebaseQueryPrompt(input: CodebaseQueryInput): string {
  const { query, submissions, context } = input;
  
  // Send full code for better analysis (AI can handle it)
  const codeSubmissions = submissions.map((sub, idx) => 
    `=== Submission ${idx + 1} (ID: ${sub.id}) ===\n${sub.code}\n`
  ).join('\n\n');
...
export function generateSubmissionComparisonPrompt(input: SubmissionComparisonInput): string {
...
export function generateSmartSearchPrompt(input: SmartSearchInput): string {
  const { query, submissions } = input;
  
  const submissionSummaries = submissions.map(sub => 
    `ID: ${sub.id}, Status: ${sub.metadata?.status}, Issues: ${sub.results?.findings?.length || 0}`
  ).join('\n');
